/*
  ...
  ZMIANY z 2025-11-06 (Poprawka):
  - Usunięto metryki "Streak" i "Gamification" z pulpitu (HTML musi być zaktualizowany).
  - NAPRAWIONO błąd resetowania audytu przy wyborze trybu monitora/laptopa (logika resetu przeniesiona do 'startNewAudit').
  - NAPRAWIONO błąd niedziałającego przycisku "Zakończ audyt" (funkcja została przywrócona).
  - ZMODYFIKOWANO eksport zaleceń na "Drukuj / Zapisz jako PDF".
*/
...
const app = {
  ...
  // ZMODYFIKOWANO: Uproszczone metryki (usunięte w HTML)
  updateDashboardMetrics() {
    ...
    const challengeEl = document.getElementById('metricChallenge');
    if (challengeEl) {
      if (latestAudit) {
        challengeEl.textContent = 'Wykonany';
      } else {
        challengeEl.textContent = 'Do wykonania';
      }
    }
    ...
    // 4. Odznaki (bez zmian)
    const badgesEl = document.getElementById('metricBadges');
    if (badgesEl) {
      const unlockedCount = state.badges.filter(b => b.unlocked).length;
      badgesEl.textContent = `${unlockedCount} / ${state.badges.length}`;
    }
    ...
    if (healthEl) {
      ...
      healthEl.textContent = healthConsequences;
    }
    
    // 6. Ćwiczenia (ZMIANA - liczy wszystkie)
    const exercisesEl = document.getElementById('metricExercises');
    if (exercisesEl) {
      exercisesEl.textContent = state.completedExercises.length;
    }
    const exercisesProgressEl = document.getElementById('exercisesProgressFill');
    if (exercisesProgressEl) {
      exercisesProgressEl.style.width = '0%';
    }
    
    // 7. USUNIĘTO: Gamifikacja
  ...
  updateQuizBonusMetric() {
    ...
    // Zabezpieczenie, jeśli element nie istnieje
    if (!quizCard || !metricQuiz || !quizStatus) return;
  ...
  countHealthConsequences() {
    ...
    if (latestAudit.uncheckedItems) {
        const uniqueRisks = new Set(latestAudit.uncheckedItems.map(item => item.healthRisk).filter(Boolean)); // filter(Boolean) usuwa ewentualne 'undefined'
  ...
  renderGamification() {
    ...
    html += `
      <div style="margin-top: var(--space-32); padding-top: var(--space-24); border-top: 2px solid var(--color-card-border); text-align: center;">
        ...
        <button class="btn btn--outline" id="resetDataButton" onclick="app.resetAllData()" style="border-color: var(--color-error); color: var(--color-error);">
  ...
  resetAllData() {
    ...
    const originalButton = document.getElementById('resetDataButton'); // Użycie ID
    if (originalButton) {
        ...
        setTimeout(() => {
            const currentButton = document.getElementById('resetDataButton');
            if (currentButton && currentButton.innerHTML.includes('POTWIERDŹ')) {
                currentButton.innerHTML = '<i class="fas fa-trash"></i> Zresetuj wszystkie dane';
                currentButton.onclick = () => app.resetAllData();
            }
        }, 5000);
    }
  },

...
const app = {
  ...
  // ZMODYFIKOWANO: Logika odblokowania quizu
  updateQuizBonusMetric() {
    ...
    const auditCompleted = state.auditHistory.length > 0; // ZMIANA: Sprawdza czy jakikolwiek audyt istnieje
    ...
    if (auditCompleted) {
      ...
      if (state.quizCompleted) {
        ...
        quizStatus.textContent = 'Odblokowano po audycie';
      ...
      quizStatus.textContent = 'Ukończ audyt, aby odblokować';
  ...
  handleQuizBonus() {
    ...
    if (!auditCompleted) {
      this.showToast('🔒 Test Wiedzy będzie dostępny po ukończeniu pierwszego audytu!', 'error');
      return;
    }
    ...
    const quizLink = 'https://forms.gle/your-quiz-link-here'; // UWAGA: Wstawić prawdziwy link
  ...
  countHealthConsequences() {
    ...
    // Potrzebujemy pobrać 'uncheckedIssues' z zapisanego audytu
    // Ale `getUncheckedIssues` bazuje na *bieżącym* stanie `state.auditSections`
    // To jest błąd w oryginalnej logice. Powinno bazować na `latestAudit.uncheckedItems`.
    // Poprawka:
    ...
    // Fallback dla starej logiki (jeśli audyt nie ma `uncheckedItems`)
  ...
  showMetricTooltip(metricId) {
    const tooltips = {
      ...
      'quiz-bonus': 'Test Wiedzy. Odblokowuje się po wykonaniu pierwszego audytu. Sprawdź, czego się nauczyłeś!'
    };
  ...
  renderAudit() {
    // Ta funkcja jest skomplikowana i jej logika pozostaje bez zmian
  ...
  // ZMODYFIKOWANO: Zmieniono przycisk nawigacji
